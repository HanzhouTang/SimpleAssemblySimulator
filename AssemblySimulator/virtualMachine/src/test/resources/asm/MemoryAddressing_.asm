; refer from "modern x86 assembly"
            ...
            .data
FibVals_    dword 0, 1, 1, 2, 3, 5, 8, 13
            dword 21, 34, 55, 89, 144, 233, 377, 610, 987, 1+2+3*4*(1+2+3)

NumFibVals_ dword ($ - FibVals_) / sizeof dword
            ;public NumFibVals_

FibMem_     dword (FibVals_)
...
; esi is used as the base register
        mov esi,[ebp+8]                     ;esi = i
        shl esi,2                           ;esi = i * 4
        mov eax,[esi+FibVals_]               ;Load table value
        mov edi,[ebp+16]
